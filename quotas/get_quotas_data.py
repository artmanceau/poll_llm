...
def get_quotas(N=100):
    codes = (
        ...
        .select("libelle français", "code")
        .with_columns(pl.col("code").str.split_exact("-", 1).alias("code_parts"))
        .unnest("code_parts")
        .rename({"field_0": "GEO_OBJECT", "field_1": "GEO", "libelle français": "name"})
    )

    codes_communes = (
        codes.rename({"name": "commune"})
        .filter(pl.col("GEO_OBJECT") == "COM")
        .select("GEO", "commune")
    )
    # codes_dep = (
    #     codes.rename({"name": "dep"})
    #     .filter(pl.col("GEO_OBJECT") == "DEP")
    #     .select("GEO", "dep")
    # )
    ...
    df = (
        ...
        .with_columns(pl.col("GEO").cast(pl.String))
        .join(codes_communes, on="GEO")
        .collect()
    )
    ...
    sample = (
        ...
        .with_columns(pl.col("dep_").cast(pl.String).replace(DEPARTEMENT).alias("dep"))
        .select("commune", "AGE", "SEX", "PCS", "dep")
    )
    return sample
