...
version = st.sidebar.selectbox(
    "Version",
    sorted(
        results.filter(pl.col("VERSION") >= MIN_VERSION)["VERSION"].unique().to_list(),
        reverse=True,
    ),
)


r = results.filter(pl.col("VERSION") == version)


model = st.sidebar.selectbox(
    "Modèle",
    sorted(r["MODEL"].unique().to_list()),
)


r = r.filter(pl.col("MODEL") == model)


year = st.sidebar.selectbox(
    "Année",
    sorted(
        r["YEAR"].unique().to_list(),
        reverse=True,
    ),
)


r = r.filter(pl.col("YEAR") == year)


respondents = st.sidebar.selectbox(
    "Répondants",
    sorted(
        r.filter(pl.col("N_RESPONDENTS") > MIN_N)["N_RESPONDENTS"].unique().to_list()
    ),
)
...
with tab_main:
    ...
    if "probabilite" in detail.columns:
        st.header("Taux de participation")

        st.info(
            "Le taux de participation est calculée comme la note moyenne sur une echelle de 0 à 100 que les répondants ont indiqué à propos de leur intention d'aller voter"
        )

        true_ppar = {2022: 0.8050, 2017: 0.8361}
        if year in true_ppar:
            ppar = true_ppar[year]
            st.metric(
                value=ppar,
                label="Taux de participation réel",
                delta_color="green",
                format="percent",
            )
        else:
    ...
    # Remove non-vote
    resume_wo_abstention = resume.filter(
        ~(
            pl.col(f"vote{year}").is_in(
                ["Abstention", "Non inscrit", "Vote blanc ou nul"]
            )
        )
    )
    ...
    with st.expander("📊 Analyse socio-démographique"):
        rows = [
            st.columns(2),
            st.columns(2),
        ]
    ...
    with st.expander("💬 Raisons de vote"):
        ...
        candidat_df = detail.filter(pl.col(f"vote{year}") == candidat)
        ...
        with c4:
            geo = st.selectbox(
                "dep",
                sorted(candidat_df["dep"].unique().to_list()),
                index=None,
                key="filter_geo",
            )
        ...
        if age is not None:
            resultats = resultats.filter(pl.col("AGE") == age)

        if pcs is not None:
            resultats = resultats.filter(pl.col("PCS") == pcs)

        if sexe is not None:
            resultats = resultats.filter(pl.col("SEX") == sexe)

        if geo is not None:
            resultats = resultats.filter(pl.col("dep") == geo)

        st.write(f"**{resultats.height} répondants correspondants**")
...
with tab_bias:
    ...
    if official.is_empty():
        ...
        with st.spinner("Chargement de toutes les simulations…"):
            all_summaries = load_all_summaries(year, MIN_VERSION, MIN_N)

        if all_summaries.is_empty():
            st.warning("Aucune simulation trouvée pour cette année.")
